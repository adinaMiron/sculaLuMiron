...
| **5161–5216** | **Importance markers** — `renderImportance` (the pill; `data-i` on the label in the preview, baked in for the export), `impSetLine` (put the marker after the bullet / `[ ]` / hashes / assignee, replace or remove), `setImportance` (used by Ctrl+Alt+0..3), `impFind` (a click on a pill searches for its own level). `filterImportance` near the workbook filters powers `#importance-select` across all workbooks and the open chapter preview. |
