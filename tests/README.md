...
`recipes.js`, `graph.js`, `find.js`, `nav.js`, `wbrename.js`, `paste.js` and `voice.js` are the scripts that do **not** use
...
in-memory document, so none depends on a write landing. `voice.js` stays
