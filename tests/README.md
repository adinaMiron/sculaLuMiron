...
`graph.js`, `find.js`, `nav.js`, `wbrename.js`, `wbsaveall.js` and `paste.js` — for
...
pending-edit tracking under it, and pasting a picture
into it, plus — in `voice.js` — `index.html`'s keep-the-audio
...
- `VOICE_URL` — the same, for `voice.js`, which drives `index.html`.
...
`recipes.js`, `graph.js`, `find.js`, `nav.js`, `wbrename.js`, `wbsaveall.js`, `paste.js` and `voice.js` are the scripts that do **not** use
