---
description: Parse-check the JS in all four app files, confirm the nav block is in sync, and check diacritics
---
...
## 2. The shared nav block is byte-identical in all four files
...
If the nav drifted, the fix is to re-apply the change to all four, not to
